...
func BookHandler(meTruyencvClient *resty.Client, myTruyenClient *resty.Client, bookID int, workerID int) bool {
	...
	if (err != nil || bookResponse.StatusCode() != 200) && (bookResponse == nil || bookResponse.StatusCode() != 404) {
		return false
	}
	...
	if err == nil && bookResponse.StatusCode() == 200 {
		...
		updated, updateErr := myTruyenClient.R().
			SetBody(updatePayload).
			Patch(fmt.Sprintf("books/id/%d", bookID))
		if updateErr != nil || updated.IsError() {
			log.Printf("[Worker %d] Failed to update book %d", workerID, bookID)
			return false
		}
	} else {
		// Handle author
