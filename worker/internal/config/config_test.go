...
func TestLoad(t *testing.T) {
	...
	if c, err := Load(func(k string) string { return valid[k] }); err != nil || c.Concurrency != 2 {
		t.Fatalf("defaults: %v", err)
	}
	for key, value := range map[string]string{"MYTRUYEN_BACKEND": "file:///tmp", "MYTRUYEN_PASSWORD": "", "RABBITMQ_URL": "https://localhost", "RABBITMQ_QUEUE_CRAWL": "", "CRAWL_CONCURRENCY": "0", "HTTP_TIMEOUT": "-1s"} {
